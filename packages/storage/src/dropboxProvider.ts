...
export interface DropboxProviderOptions {
  /** Meldet eine erfolgreiche, adapterseitige Erneuerung des Access-Tokens. */
  onTokenRefreshed?: (connectionId: string) => void
}
  ...
  return {
    ...
    async login(state?: StorageCommandState): Promise<void> {
      ...
      localStorage.setItem(
        authStateKey(connectionId),
        JSON.stringify({ state: oauthState, verifier }),
      )
    ...
    async list(path: StorageCommandState, recursive = false): Promise<string[]> {
      ...
      const entries = await listDropboxEntries(
        connectionKey(path),
        folder,
        recursive,
    ...
    async search(path: StorageCommandState, query: string): Promise<string[]> {
      const folder = resolveStorageFolder(path)
      const response = await authenticatedDropboxFetch(
        connectionKey(path),
      ...
      if (!response.ok) {
        throw new Error(
          `Dropbox search failed: ${response.status} ${await readDropboxErrorMessage(response)}`,
        )
      }
    ...
    async open(path: StorageCommandState, filename: string): Promise<string | undefined> {
      ...
      if (!response.ok) {
        throw new Error(
          `Dropbox open failed: ${response.status} ${await readDropboxErrorMessage(response)}`,
        )
      }
    ...
    async save(path: StorageCommandState, filename: string, content: string | Blob): Promise<void> {
      ...
      if (!response.ok) {
        throw new Error(
          `Dropbox save failed: ${response.status} ${await readDropboxErrorMessage(response)}`,
        )
      }
    },
    ...
    async listFolders(
      ...
      return entries
        ...
        .map((entry) => ({
          name: entry.name,
          path: normalizeFolderPath(entry.path_display ?? joinPath(folder, entry.name)),
        }))
    ...
    async listDocuments(state: StorageCommandState): Promise<StorageDocument[]> {
      const entries = await listDropboxEntries(
        connectionKey(state),
        resolveStorageFolder(state),
        false,
        options.onTokenRefreshed,
      )
      const files = entries.filter(
        (entry) => entry['.tag'] === 'file' && !entry.name.startsWith('.'),
      )
      return files
    ...
    async openPreview(state: StorageCommandState, path: string): Promise<Blob | undefined> {
      ...
      const response = await authenticatedDropboxFetch(
        connectionKey(state),
        'https://content.dropboxapi.com/2/files/download',
        { method: 'POST', headers: { 'Dropbox-API-Arg': JSON.stringify({ path: `/${target}` }) } },
        options.onTokenRefreshed,
      )
      if (!response.ok)
        throw new Error(
          `Dropbox PDF preview failed: ${response.status} ${await readDropboxErrorMessage(
            response,
          )}`,
        )
...
async function listDropboxEntries(
  connectionId: string,
  folder: string,
  recursive: boolean,
  onTokenRefreshed: DropboxProviderOptions['onTokenRefreshed'],
): Promise<DropboxEntry[]> {
  const firstResponse = await authenticatedDropboxFetch(
    connectionId,
  ...
  if (!firstResponse.ok) {
    throw new Error(
      `Dropbox list failed: ${firstResponse.status} ${await readDropboxErrorMessage(
        firstResponse,
      )}`,
    )
  }
  ...
  while (hasMore) {
    const continueResponse = await authenticatedDropboxFetch(
      connectionId,
    ...
    if (!continueResponse.ok) {
      throw new Error(
        `Dropbox list failed: ${continueResponse.status} ${await readDropboxErrorMessage(
          continueResponse,
        )}`,
      )
    }
...
async function authenticatedDropboxFetch(
  connectionId: string,
  url: string,
  init: RequestInit,
  onTokenRefreshed: DropboxProviderOptions['onTokenRefreshed'],
): Promise<Response> {
  const token = await validDropboxToken(connectionId, onTokenRefreshed)
  ...
  const refreshedToken = await refreshDropboxToken(connectionId, token, onTokenRefreshed)
...
async function validDropboxToken(
  ...
  if (
    ...
    return refreshDropboxToken(connectionId, token, onTokenRefreshed)
  }
  return token
}
...
async function refreshDropboxToken(
  connectionId: string,
  previousToken: DropboxTokenResponse,
  onTokenRefreshed: DropboxProviderOptions['onTokenRefreshed'],
): Promise<DropboxTokenResponse> {
  ...
  if (!response.ok) {
    throw new Error(
      `Dropbox token refresh failed: ${response.status} ${await readDropboxErrorMessage(response)}`,
    )
  }
...
function persistDropboxToken(connectionId: string, token: DropboxTokenResponse): void {
  const expiresAt =
    token.expires_in === undefined ? token.expiresAt : Date.now() + token.expires_in * 1000
  localStorage.setItem(tokenKey(connectionId), JSON.stringify({ ...token, expiresAt }))
}

async function fetchWithDropboxToken(
  url: string,
  init: RequestInit,
  accessToken: string,
): Promise<Response> {
