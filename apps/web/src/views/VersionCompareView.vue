...
import { extractSongConfig, splitSongDocument } from '@zupfnoter/core'
import InlineTextDiff from '../components/InlineTextDiff.vue'
...
const mode = ref<CompareMode>('blink')
...
const hasTextOutput = computed(() => kind.value === 'abc' || kind.value === 'config')
const textDiffValues = computed(() => {
  const currentRequest = request.value
  if (currentRequest === undefined || !hasTextOutput.value) return { oldText: '', newText: '' }
  return {
    oldText: kind.value === 'abc'
      ? splitSongDocument(currentRequest.referenceText).abcText
      : JSON.stringify(extractSongConfig(currentRequest.referenceText), null, 2),
    newText: kind.value === 'abc'
      ? splitSongDocument(currentRequest.comparisonText).abcText
      : JSON.stringify(extractSongConfig(currentRequest.comparisonText), null, 2),
  }
})
...
<template>
  <main class="version-compare">
    ...
    <section class="version-compare__stage" aria-label="Versionsvergleich">
      <section v-if="hasTextOutput" class="version-compare__text-diff" :aria-label="`${kindLabel}-Vergleich`">
        <header><h2>{{ kindLabel }}</h2><span>{{ selectedChoice?.label ?? 'Gesamter Inhalt' }}</span></header>
        <InlineTextDiff :old-text="textDiffValues.oldText" :new-text="textDiffValues.newText" :old-label="referenceTitle" :new-label="comparisonTitle" />
      </section>
      <div v-else-if="hasVisualOutput && mode === 'side-by-side'" class="version-compare__side-by-side">
      ...
      <div v-else-if="hasVisualOutput" class="version-compare__single">
        <article ref="referencePane" @scroll="syncPaneScroll($event, comparisonPane)" :class="{ 'is-visible': mode !== 'blink' || blinkVisible === 'reference' }" :aria-hidden="mode === 'blink' && blinkVisible !== 'reference'"><h2>{{ referenceTitle }}</h2><object v-if="kind === 'pdf' && referencePdfUrl" :data="referencePdfUrl" type="application/pdf" class="version-compare__pdf" aria-label="PDF der Referenzversion" /><div v-else-if="referenceSvg" v-html="referenceSvg" /></article>
        <article ref="comparisonPane" @scroll="syncPaneScroll($event, referencePane)" class="version-compare__comparison-layer" :class="{ 'is-visible': mode !== 'blink' || blinkVisible === 'comparison' }" :style="mode === 'swipe' ? { clipPath: `inset(0 ${100 - swipePosition}% 0 0)` } : undefined" :aria-hidden="mode === 'blink' && blinkVisible !== 'comparison'"><h2>{{ comparisonTitle }}</h2><object v-if="kind === 'pdf' && comparisonPdfUrl" :data="comparisonPdfUrl" type="application/pdf" class="version-compare__pdf" aria-label="PDF der Vergleichsversion" /><div v-else-if="comparisonSvg" v-html="comparisonSvg" /></article>
        <div v-if="mode === 'swipe'" class="version-compare__swipe-handle" :style="{ left: `${swipePosition}%` }" />
      </div>
      <p v-else class="version-compare__not-visual">{{ kindLabel }} ist als Auswahl vorbereitet. Die inhaltliche Gegenüberstellung folgt separat.</p>
    </section>
  </main>
</template>

<style scoped>
.version-compare{display:grid;grid-template-columns:minmax(19rem,24rem) minmax(0,1fr);height:100vh;min-height:0;overflow:hidden;background:#e2e8f0;color:#24364d}.version-compare__sidebar{display:flex;flex-direction:column;gap:1rem;box-sizing:border-box;min-height:0;padding:1rem;border-right:1px solid #b8c5d5;background:rgba(255,255,255,.98);overflow:auto}.version-compare__card{display:grid;gap:.7rem;padding:1rem;border:1px solid #c8d3e0;border-radius:.8rem;background:#f8fafc;box-shadow:0 8px 24px rgba(36,54,77,.08)}.version-compare__card--selection{background:#eef5fc;border-color:#9fb9d4}.version-compare__kicker{margin:0 0 .35rem;color:#3269a8;font-size:.76rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase}.version-compare h1,.version-compare h2{margin:0}.version-compare h1{font-size:1.45rem}.version-compare h2{font-size:1rem}.version-compare__path{margin:.55rem 0 0;overflow-wrap:anywhere;color:#526982;font-size:.82rem}.version-compare__version{display:grid;gap:.15rem}.version-compare__version span,.version-compare__meta,.version-compare label span,.version-compare__hint{color:#526982;font-size:.78rem}.version-compare__version strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600}.version-compare label{display:grid;gap:.3rem}.version-compare select,.version-compare input{width:100%;box-sizing:border-box;padding:.45rem;border:1px solid #b8c5d5;border-radius:.4rem;background:white;color:inherit;font:inherit}.version-compare__hint,.version-compare__status,.version-compare__error,.version-compare__not-visual{margin:0}.version-compare__blink-actions{display:flex;gap:.45rem}.version-compare button{padding:.4rem .65rem;border:1px solid #9eb2c9;border-radius:.4rem;background:white;color:inherit;font:inherit;cursor:pointer}.version-compare button:hover{background:#e7eef7}.version-compare__status{color:#3269a8;font-size:.82rem}.version-compare__error{padding:.7rem;border:1px solid #e8a4a4;border-radius:.4rem;color:#a52626;background:#fff5f5}.version-compare__stage{min-width:0;min-height:0;height:100%;padding:1rem;overflow:auto}.version-compare__side-by-side{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;min-width:0}.version-compare__side-by-side article,.version-compare__single article{min-width:0;padding:.8rem;border:1px solid #c8d3e0;border-radius:.8rem;background:#fff;box-shadow:0 8px 24px rgba(36,54,77,.08)}.version-compare__side-by-side h2,.version-compare__single h2{padding-bottom:.6rem;border-bottom:1px solid #d6dee8}.version-compare__single h2{position:sticky;top:-.8rem;z-index:1;margin:-.8rem -.8rem .6rem;padding:.8rem;background:#fff;border-bottom:1px solid #d6dee8}.version-compare__side-by-side :deep(svg),.version-compare__single :deep(svg){display:block;width:100%;height:auto}.version-compare__pdf{display:block;width:100%;height:calc(100vh - 7rem);min-height:40rem;border:0}.version-compare__single{position:relative;min-height:calc(100vh - 2rem);overflow:hidden}.version-compare__single article{position:absolute;inset:0;overflow:auto}.version-compare__single article:not(.is-visible){visibility:hidden}.version-compare__comparison-layer{z-index:2}.version-compare__swipe-handle{position:absolute;z-index:3;top:0;bottom:0;width:2px;transform:translateX(-1px);background:#3269a8;pointer-events:none}.version-compare__single article:first-child{z-index:1}.version-compare__single article.is-visible{visibility:visible}.version-compare__text-diff{display:flex;flex-direction:column;min-height:0;height:100%;box-sizing:border-box;padding:1rem;border:1px solid #c8d3e0;border-radius:.8rem;background:#fff;box-shadow:0 8px 24px rgba(36,54,77,.08)}.version-compare__text-diff header{flex:0 0 auto;display:flex;align-items:baseline;justify-content:space-between;gap:1rem;padding-bottom:.7rem;border-bottom:1px solid #d6dee8}.version-compare__text-diff header span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#526982;font-size:.82rem}.version-compare__text-diff :deep(.inline-text-diff){flex:1 1 auto;min-height:0;margin-top:1rem}.version-compare__not-visual{padding:2rem;color:#526982}@media(max-width:60rem){.version-compare{grid-template-columns:1fr;height:auto;min-height:100vh;overflow:visible}.version-compare__sidebar{border-right:0;border-bottom:1px solid #b8c5d5}.version-compare__stage{height:auto;min-height:70vh}.version-compare__single{min-height:70vh}}@media(max-width:42rem){.version-compare__side-by-side{grid-template-columns:1fr}}
