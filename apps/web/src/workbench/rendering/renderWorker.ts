...
self.onmessage = (event: MessageEvent<RenderWorkerRequest>) => {
  const { id, abcText, extractNr, practiceQrJpegUrl, resources, flowconf } = event.data
  ...
  try {
    ...
    const result = renderWorkbenchPreviews(abcText, extractNr, { practiceQrJpegUrl, resources, flowconf })
