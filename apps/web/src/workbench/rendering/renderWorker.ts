...
self.onmessage = (event: MessageEvent<RenderWorkerRequest>) => {
  const { id, abcText, extractNr, playerQrJpegUrl, resources, flowconf } = event.data
  ...
  try {
    ...
    const result = renderWorkbenchPreviews(abcText, extractNr, { playerQrJpegUrl, resources, flowconf })
